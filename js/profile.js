...
export async function loadProfile(username) {
    ...
    document.getElementById('profile-avatar').src = '/assets/logo.svg';
            ...
            if (tracks.length > 0) {
                ...
                recentContainer.innerHTML = tracks
                    .map((track, index) => {
                        ...
                        if (!image) image = '/assets/logo.svg';
                        ...
                        <div class="track-item lastfm-track" data-title="${escapeHtml(track.name)}" data-artist="${escapeHtml(track.artist?.['#text'] || track.artist?.name || '')}" style="grid-template-columns: 40px 1fr auto; cursor: pointer;">
                            <img id="${track._imgId}" src="${image}" class="track-item-cover" style="width: 40px; height: 40px; border-radius: 4px;" loading="lazy" onerror="this.src='/assets/logo.svg'">
                    ...
                    .map((artist, index) => {
                        ...
                        <div class="card artist lastfm-card" data-name="${escapeHtml(artist.name)}" style="cursor: pointer;">
                            <div class="card-image-wrapper">
                                <img id="${imgId}" src="${image}" class="card-image" loading="lazy" onerror="this.src='/assets/logo.svg'">
                            </div>
                            <div class="card-info">
                ...
                topTracksContainer.innerHTML = tracks
                    .map((track, index) => {
                        ...
                        <div class="track-item lastfm-track" data-title="${escapeHtml(track.name)}" data-artist="${escapeHtml(artistName)}" style="grid-template-columns: 40px 1fr auto; cursor: pointer;">
                            <img id="${imgId}" src="${image}" class="track-item-cover" style="width: 40px; height: 40px; border-radius: 4px;" loading="lazy" onerror="this.src='/assets/logo.svg'">
                            <div class="track-item-info">
                                <div class="track-item-details">
                                    <div class="title">${track.name}</div>
    ...
    if (profile.privacy?.playlists !== 'private' || isOwner) {
        ...
        Object.values(playlists).forEach((playlist) => {
            ...
            card.innerHTML = `
                <div class="card-image-wrapper">
                    <img src="${playlist.cover || '/assets/logo.svg'}" class="card-image" loading="lazy" alt="${playlist.name}">
                </div>
                <div class="card-info">
